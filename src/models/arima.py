...
FIX:
  ...
  are handled gracefully by the Kalman filter inside SARIMAX
...
class SARIMAForecaster(BaseForecaster):
    ...
    def load(cls, path: str | Path) -> "SARIMAForecaster":
        ...
        instance._model          = state.get("model_wrapper")
        instance.is_fitted = True
        return instance

    # ── internal ──────────────────────────────────────────────────────────

    def _extract_series(self, df: pd.DataFrame, reindex: bool = False) -> pd.Series:
        """Pull target column and optionally reindex to a contiguous hourly timeline.

        The feature pipeline drops rows that have missing exogenous features,
        which leaves gaps in the DatetimeIndex.  SARIMA (and any other model
        that relies on physical time spacing) will misinterpret a 2-hour gap
        as two consecutive 1-hour steps.  Setting reindex=True fills those
        gaps back in with NaN, restoring the correct hourly frequency before
        the series is handed to statsmodels.

        NaN values — whether from reindexing or naturally present in df — are
        kept in the returned series so its length always equals len(df).  This
        satisfies the BaseForecaster contract (predictions must match len(df))
        and prevents the array-shape mismatch that caused a ValueError in
        metrics.py.  The SARIMAX Kalman filter handles NaN observations natively
        (enforce_stationarity=False / enforce_invertibility=False).
        """
        if reindex and isinstance(df.index, pd.DatetimeIndex):
            full_idx = pd.date_range(df.index.min(), df.index.max(), freq="h")
            s = df[self.target_col].reindex(full_idx)
        else:
            s = df[self.target_col]
        if s.notna().sum() < self.horizon * 2:
            raise ValueError(
                f"Too few non-NaN observations ({s.notna().sum()}) for SARIMA fit."
            )
        return s
