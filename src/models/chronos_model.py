...
class ChronosForecaster(BaseForecaster):
    ...
    def _load_pipeline(self, checkpoint: str | Path | None = None) -> None:
        ...
        self._logger.info("Loading ChronosPipeline from '%s' on %s …", source, self.device)
    ...
    def fit(
        ...
        train_series = train_df[self.target_col].dropna()
        val_series   = val_df[self.target_col].dropna() if val_df is not None else None

        self._logger.info(
            "Fine-tuning Chronos on '%s': %d training obs, context=%d, horizon=%d …",
            self.target_col, len(train_series), self.context_length, self.horizon,
        )
        ...
        train_dataset = _make_sliding_windows(
            train_series.values.astype(np.float32),
            context_length=self.context_length,
            horizon=self.horizon,
        )
        val_dataset = (
            _make_sliding_windows(
                val_series.values.astype(np.float32),
                context_length=self.context_length,
                horizon=self.horizon,
            )
        ...
        total_steps = cfg["num_epochs"] * max(1, len(train_dataset) // cfg["batch_size"])
        ...
        for epoch in range(cfg["num_epochs"]):
            ...
            if val_dataset is not None:
                ...
                if val_loss < best_val_loss:
                    ...
                    self._logger.info("    ✓ New best val_loss — checkpoint saved.")
                else:
                    patience_counter += 1
                    if patience_counter >= cfg["early_stopping_patience"]:
                        self._logger.info("  Early stopping at epoch %d.", epoch + 1)
                        break
            else:
                self._logger.info(
                    "  Epoch %d/%d — train_loss=%.4f", epoch + 1, cfg["num_epochs"], train_loss
                )

        # Reload best checkpoint
        if val_dataset is not None and (checkpoint_dir / "config.json").exists():
            self._logger.info("Reloading best checkpoint from %s …", checkpoint_dir)
    ...
    def predict(self, df: pd.DataFrame) -> np.ndarray:
        ...
        # ChronosPipeline.predict returns (N, num_samples, horizon)
        ...
        forecast_np = forecast.numpy()              # (N, num_samples, H)
    ...
    def predict_quantiles(
        ...
        with torch.inference_mode():
            forecast = self._pipeline.predict(
                context=context_tensor,
                prediction_length=self.horizon,
                num_samples=max(self.num_samples, 100),   # more samples for quantiles
                limit_prediction_length=False,
            )

        forecast_np = forecast.numpy()   # (N, samples, H)
        return {
