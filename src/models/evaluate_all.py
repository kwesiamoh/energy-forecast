...
def run_all_evaluations(
    ...
    _eval_xgboost(targets, models_dir, test, registry, horizon)

    # ── 2. SARIMA ─────────────────────────────────────────────────────────
    if not skip_sarima:
        logger.info("=" * 56)
        logger.info("Evaluating SARIMA …")
        _eval_sarima(targets, models_dir, test, registry, horizon)

    # ── 3. Chronos zero-shot ──────────────────────────────────────────────
    if not skip_chronos:
        logger.info("=" * 56)
        logger.info("Evaluating Chronos (zero-shot) …")
        _eval_chronos_zeroshot(targets, test, registry, horizon)

    # ── 4. Chronos fine-tuned ─────────────────────────────────────────────
    if not skip_chronos:
        logger.info("=" * 56)
        logger.info("Evaluating Chronos (fine-tuned) …")
        _eval_chronos_finetuned(targets, models_dir, test, registry, horizon)
...
def _eval_xgboost(targets, models_dir, test, registry, horizon):
    try:
    ...
    for target in targets:
        ...
        result = model.evaluate(test, split_name="test")
        registry.add(result)
        _save_horizon_plot(model, test, target, "xgboost", horizon,
                           out_dir=Path("results/leaderboard_plots"))


def _eval_sarima(targets, models_dir, test, registry, horizon):
    try:
    ...
    for target in targets:
        ...
        preds  = model.predict_rolling(test)
...
def _eval_chronos_zeroshot(targets, test, registry, horizon):
    try:
...
def _eval_chronos_finetuned(targets, models_dir, test, registry, horizon):
    try:
        from src.models.chronos_model import ChronosForecaster
    except ImportError:
        return
