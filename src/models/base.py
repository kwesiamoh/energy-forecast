...
────────
  ...
  evaluate(df, target_col, horizon, split_name)
      ...
      Accepts an optional history_df for cold-start warmup — see
      the evaluate() docstring for details.
...
class BaseForecaster(ABC):
    ...
    def evaluate(
        ...
        Args:
            ...
            history_df: Optional warmup DataFrame prepended to df before
                        calling predict().  Sequence models (SARIMA, Chronos)
                        need at least context_length rows of history to
                        produce meaningful forecasts at the start of a new
                        split; without this they "cold-start" blind.

                        When provided, predict() receives
                        pd.concat([history_df, df]), and the predictions for
                        the warmup rows are discarded so metrics are computed
                        only on the target split rows.

                        Typical usage: pass the validation split as history_df
                        when evaluating the test split.
        ...
        if history_df is not None and len(history_df) > 0:
            ...
            all_preds = self.predict(combined)        # (N_history + N, H)
            n_warmup  = len(history_df)
            preds     = all_preds[n_warmup:]          # (N, H) — target split only
        else:
