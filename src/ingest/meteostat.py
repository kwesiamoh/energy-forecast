...
DE_STATIONS = [
    ...
    ("sylt",         "10018", 54.91,  8.33,  26, "North Sea island, max wind exposure"),
    ("schleswig",    "10035", 54.53,  9.55,  43, "Schleswig-Holstein interior"),
    ("hamburg",      "10147", 53.63,  9.99,  15, "Hamburg metro, port load centre"),
    # ── Baltic coast & Mecklenburg ────────────────────────────────────────
    ("rostock",      "10170", 54.18, 12.08,  16, "Baltic coast, offshore wind hub"),
    ("greifswald",   "10184", 54.10, 13.40,  20, "Northeast Baltic, wind corridor"),
    # ── Brandenburg & Berlin ──────────────────────────────────────────────
    ("berlin",       "10384", 52.47, 13.40,  50, "Berlin metro, continental east"),
    ("cottbus",      "10393", 51.78, 14.32,  69, "Lusatia, high solar, brown-coal legacy"),
    # ── Lower Saxony & Bremen ─────────────────────────────────────────────
    ("bremerhaven",  "10107", 53.52,  8.58,   4, "North Sea estuary, offshore wind"),
    ("hannover",     "10338", 52.47,  9.70,  56, "Central Lower Saxony"),
    # ── North Rhine-Westphalia ────────────────────────────────────────────
    ("essen",        "10410", 51.40,  6.97,  60, "Ruhr metro, highest load density DE"),
    ("koeln",        "10513", 50.87,  7.17,  92, "Cologne / Rhine lowlands"),
    # ── Hesse & Thuringia ────────────────────────────────────────────────
    ("frankfurt",    "10637", 50.05,  8.60, 111, "Main economic hub, central DE"),
    ("erfurt",       "10452", 50.98, 10.97, 316, "Thuringia basin, central uplands"),
    # ── Saxony ───────────────────────────────────────────────────────────
    ("dresden",      "10488", 51.13, 13.75, 227, "Saxony, continental east, solar belt"),
    # ── Rhineland-Palatinate & Saarland ──────────────────────────────────
    ("trier",        "10609", 49.75,  6.66, 265, "Mosel valley, SW Germany"),
    # ── Baden-Wuerttemberg ───────────────────────────────────────────────
    ("stuttgart",    "10739", 48.69,  9.22, 371, "Southwest, wine climate, high solar"),
    ("freiburg",     "10803", 47.99,  7.85, 237, "Upper Rhine, Germany's sunniest city"),
    # ── Bavaria ──────────────────────────────────────────────────────────
    ("nuernberg",    "10763", 49.50, 11.05, 312, "Franconia, central Bavaria"),
    ("munich",       "10866", 48.35, 11.79, 447, "Munich metro, Alpine foothills"),
    ("zugspitze",    "10961", 47.42, 10.98,2960, "Alpine high altitude, foehn signal"),
]

KEEP_COLS = ["temp", "dwpt", "rhum", "prcp", "wspd", "wpgt", "pres", "tsun"]

# ── Spatial weights (Voronoi-area proxy) ─────────────────────────────────────
# Computed once: approximate fraction of Germany's area each station represents.
# Derived from a rough Voronoi tessellation of the 20 station coordinates.
# Stations near coasts / borders get smaller weights (less German territory).
STATION_WEIGHTS = {
    "sylt":        0.030,
    "schleswig":   0.055,
    "hamburg":     0.048,
    ...
    "nuernberg":   0.055,
    "munich":      0.045,
    "zugspitze":   0.015,   # alpine outlier — low weight
}
...
def _fetch_bulk_year(station_id: str, year: int) -> pd.DataFrame | None:
    ...
    URL: https://data.meteostat.net/hourly/{year}/{station_id}.csv.gz
    No API key needed. Returns None on 404.
    """
...
def _fetch_station_meteostat_lib(
    ...
    """Try the meteostat v2 library first (faster, handles gaps)."""
    try:
        ...
        logger.debug("  meteostat lib failed for %s: %s — falling back to bulk", name, exc)
        return None
...
def _fetch_station(
    ...
    Strategy:
      1. Try meteostat library (v2 API) — fast, gap-filled
    ...
    if df is not None and not df.empty:
        logger.info("  %-14s (%s): %d rows via meteostat lib", name, station_id, len(df))
        return df

    # Fall back to bulk CSV
    ...
    logger.info("  %-14s (%s): %d rows via bulk CSV", name, station_id, len(df))
    return df
...
def _weighted_composite(merged: pd.DataFrame) -> pd.DataFrame:
    ...
    For each variable, computes a weighted mean across all stations that
    have non-NaN values at each timestep (so missing stations are excluded
    from the denominator rather than pulling the mean toward zero).
    """
    for var in KEEP_COLS:
        ...
        data   = merged[station_cols].values.astype(np.float64)   # (T, N)
        w_mat  = np.where(np.isfinite(data), weights[None, :], 0.0)  # zero out NaN stations
...
def load_meteostat(
    ...
    Args:
        ...
        stations:      Override station list (default = DE_STATIONS).
                       Each entry: (name, wmo_id, lat, lon, alt, desc)
    ...
    if not station_dfs:
        raise RuntimeError(
            ...
            "Check: pip install meteostat  and internet connectivity."
        )

    merged = pd.concat(station_dfs, axis=1).sort_index()

    # Short gap fill per station (<=2 h) before composite
    merged = merged.ffill(limit=2)

    # Area-weighted national composite
