...
def build_features(
    ...
    if _gen_ef:
        ...
        _clipped  = df[_gen_cols].clip(lower=0)          # no negative generation
...
def get_feature_cols(
    ...
    Exclusion rules (applied in order):
      ...
      2. SMARD overlay columns (_smard suffix) — validation-only.
...
def valid_overlap_corr(
    ...
    target_valid = sub[[c for c in target_cols if c in sub.columns]].notna().any(axis=1)
...
def _validate(df: pd.DataFrame) -> None:
    ...
    if inf_cols:
        logger.warning("Inf values found in: %s — replacing with NaN.", inf_cols)
